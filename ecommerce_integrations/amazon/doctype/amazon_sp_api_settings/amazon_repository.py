...
class AmazonRepository:
    ...
    def create_item(self, order_item) -> str:
        def create_item_group(amazon_item) -> str:
            item_group_name = amazon_item.get("AttributeSets")[
                0].get("ProductGroup")
        ...
        def create_brand(amazon_item) -> str:
            ...
            existing_brand = frappe.db.get_value(
                "Brand", filters={"brand": brand_name})
        ...
        def create_manufacturer(amazon_item) -> str:
            manufacturer_name = amazon_item.get("AttributeSets")[
                0].get("Manufacturer")
        ...
        def create_item_price(amazon_item, item_code) -> None:
            ...
            item_price.price_list_rate = (
                amazon_item.get("AttributeSets")[0].get(
                    "ListPrice", {}).get("Amount") or 0
            )
        ...
        for field_map in self.amz_setting.amazon_fields_map:
            ...
            if field_map.item_field:
                setattr(item, field_map.item_field,
                        order_item[field_map.amazon_field])
        ...
        for field_map in self.amz_setting.amazon_fields_map:
            if field_map.use_to_find_item_code:
                item_code = frappe.db.get_value(
                    "Item",
                    filters={
                        field_map.item_field: order_item[field_map.amazon_field]},
                    fieldname="item_code",
                )
                ...
                if item_code:
                    ...
                    field_label = frappe.get_meta(
                        "Item").get_label(field_map.item_field)
                    frappe.throw(
